-- Phase 3 leads ownership backfill guard.
-- Assign unowned leads only when this deployment has exactly one account.
...
COMMENT ON COLUMN leads.account_id IS
  'Nullable tenant owner for the leads ownership pilot. Guarded backfill assigns unowned rows only when exactly one account exists; ambiguous multi-account rows remain null until manual backfill. NOT NULL enforcement and RLS policies are deferred.';
