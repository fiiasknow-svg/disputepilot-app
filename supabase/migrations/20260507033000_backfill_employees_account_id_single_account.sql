-- Phase 3 employees ownership backfill guard.
-- Assign unowned employees only when this deployment has exactly one account.
...
COMMENT ON COLUMN employees.account_id IS
  'Nullable tenant owner for the employees ownership pilot. Guarded backfill assigns unowned rows only when exactly one account exists; ambiguous multi-account rows remain null until manual backfill. NOT NULL enforcement and RLS policies are deferred.';
