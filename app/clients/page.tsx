        ...
        {showForm && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
            <div style={{ background: "#fff", borderRadius: 12, padding: 28, width: 440 }}>
              ...
              {[["Full Name", "full_name"], ["Email", "email"], ["Phone", "phone"]].map(([lbl, key]) => (
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                ...
                <button onClick={save} disabled={saving} style={{ padding: "9px 20px", background: "#1e3a5f", color: "#fff", border: "none", borderRadius: 7, cursor: "pointer", fontWeight: 700 }}>{saving ? "Saving…" : "Save Client"}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "#94a3b8" }}>{search ? "No clients match your search." : "No clients yet."}</td></tr>
              ) : filtered.map(c => (
                <tr key={c.id} style={{ borderTop: "1px solid #f1f5f9" }}>
                  <td style={{ padding: "12px 16px" }}>
                    <button onClick={() => router.push(`/clients/${c.id}`)}
                      style={{ background: "none", border: "none", cursor: "pointer", fontWeight: 600, fontSize: 14, color: "#1e3a5f", padding: 0 }}>
                      {c.full_name || "—"}
                    </button>
                  </td>
                  <td style={{ padding: "12px 16px", fontSize: 14, color: "#475569" }}>{c.email || "—"}</td>
                  <td style={{ padding: "12px 16px", fontSize: 14, color: "#475569" }}>{c.phone || c.mobile_phone || "—"}</td>
                  <td style={{ padding: "12px 16px" }}>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => router.push(`/clients/${c.id}`)}
                        style={{ fontSize: 12, padding: "4px 12px", border: "1px solid #e2e8f0", borderRadius: 5, cursor: "pointer", background: "#fff" }}>View</button>
                      <button onClick={() => setDeleteId(c.id)}
                        style={{ fontSize: 12, padding: "4px 12px", border: "1px solid #fee2e2", borderRadius: 5, cursor: "pointer", background: "#fff", color: "#ef4444" }}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
