...
export default function Page() {
  ...
  async function handleProcess() {
    ...
    setProcessing(false);
    setSuccess(true);
    setClientId(""); setProcessor(""); setAmount("");
  ...
  const disabled = !clientId || !processor || !amount;
        ...
        <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.08)", padding: 32 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 18 }}>
            ...
            <div>
              ...
              <select value={processor} onChange={e => setProcessor(e.target.value)} style={sel}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 28 }}>
            ...
            <div>
              ...
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" min="0" step="0.01" style={sel} />
            </div>
          </div>

          <div style={{ display: "flex", gap: 16, marginBottom: 28 }}>
            {[
              { label: "Credit Card", icon: <CardIcon />, color: "#3b82f6", bg: "#eff6ff" },
              { label: "Cash", icon: <CashIcon />, color: "#10b981", bg: "#f0fdf4" },
              { label: "Bank Transfer", icon: <BankIcon />, color: "#8b5cf6", bg: "#f5f3ff" },
            ].map(m => (
              <div key={m.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: m.bg, border: `1px solid ${m.color}33`, borderRadius: 10, padding: "14px 20px", minWidth: 100 }}>
                {m.icon}
          ...
          <button onClick={handleProcess} disabled={processing || disabled}
            style={{ padding: "12px 40px", background: disabled ? "#cbd5e1" : "#1e3a5f", color: "#fff", border: "none", borderRadius: 7, fontSize: 15, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer" }}>
